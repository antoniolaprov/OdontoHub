...
- ✅ **Dentista / Equipe (F12)** — `GET /api/colaboradores` (leitura) +
  ...
  `POST /{nome}/disponibilidade`, `POST /{nome}/ausencia` (escrita real). Rótulo
  na UI é "Cirurgião Dentista" (não "Especialista") — só o label muda; o enum do
  backend continua `FuncaoColaborador.ESPECIALISTA`.
...
- ✅ **Dentista / Financeiro** — `GET /api/fluxo-caixa/lancamentos` + `/saldo-atual`,
  filtros de tipo (Entrada/Saída) e período (Este Mês/Mês Passado) filtram a lista
  de verdade (calculado a partir do campo `data` ISO).
- ✅ **Dentista / Prontuários (Anamnese + Plano de Tratamento, F02/F03)** —
  `GET /api/pacientes` (lista, sem botão de cadastrar — cadastro é só da
  Recepcionista), `GET/POST /api/prontuarios/anamnese/...` (responsável é um
  `<select>` de `GET /api/colaboradores` filtrado por `ESPECIALISTA`, não texto
  livre), `GET/POST/PUT/DELETE /api/prontuarios/planos/...` (procedimentos,
  encerrar, excluir). No "Realizar Procedimento": agendamento vinculado e
  executor são `<select>` (de `GET /api/agendamentos` e `/colaboradores`, não
  IDs/texto digitados); materiais usados consomem estoque de verdade via
  `POST /api/materiais/{nome}/consumo` depois do `PUT realizar`. A aba
  Prescrições (F08) continua mock — fora de escopo do F02/F03.
- ✅ **Dentista / Agenda** — tela própria (`dentista/Agenda.tsx`), somente leitura
  (mesma fonte `GET /api/agendamentos` da Recepcionista, sem criar/confirmar/
  remarcar/cancelar) — essas ações continuam exclusivas de
  `recepcionista/Agenda.tsx`.
...
- ✅ **Recepcionista / Pacientes (F13)** — `GET /api/pacientes` (lista) + `POST /api/pacientes`
  ...
  email um por vez), `POST /{nome}/restringir`, `POST /{nome}/reativar`. `temPlano`/
  `inadimplente` não existem no contexto de Cadastro de Paciente — default `false`.
- ✅ **Recepcionista / Pagamentos (F15) e Acordos (F09)** — integradas (ligadas por outro
  dev do time).
- ✅ **Auxiliar / Estoque (F05)** — `GET /api/materiais` (lista) + `POST /api/materiais`
  (cadastro de material novo — nome/unidade/saldo inicial/ponto mínimo, com validação
  de campos obrigatórios, valores não-negativos e nome duplicado), `POST /{nome}/reposicao`
  e `POST /{nome}/consumo` (consumo real disparado pelo Plano de Tratamento ao realizar
  um procedimento, rejeita quantidade ≤ 0 ou maior que o saldo). Status "OK"/"Estoque
  Baixo" é derivado (saldo ≤ ponto mínimo), não é campo do cadastro.
- ✅ **Auxiliar / Esterilização (F06) e Instrumentos (F14)** — `GET /api/instrumentos`
  ...
  `POST /{nome}/esterilizar`, `/contaminar`, `/desativar`, `/reativar`, `POST /prazo-global` e
  `POST /categorias/{categoria}/prazo` (configuração de validade). Responsável pela
  esterilização é um `<select>` de `GET /api/colaboradores` filtrado por `AUXILIAR`,
  não texto livre. Editar nome/categoria/código/tipo/prazo de um instrumento já
  cadastrado não é suportado pelo domínio — esses campos ficam somente leitura na
  edição; só o status Ativo/Inativo é real (via desativar/reativar).
...
## Permissão por perfil (UI-only, sem autorização real no backend)
Não existe sessão/role no backend (só login de clínica, F18) — as rotas `/dentista`,
`/auxiliar`, `/recepcionista` são só visuais. Por isso algumas funcionalidades são
escondidas por tela conforme o perfil, mesmo usando o mesmo dado:
- `/dentista` não tem aba Estoque (`routes.tsx`/`DentistaLayout.tsx`) — é só do Auxiliar.
- `/dentista/pacientes` (Prontuários) não tem botão de cadastrar paciente — é só da
  Recepcionista.
- `/dentista` (Agenda) é somente leitura — criar/confirmar/remarcar/cancelar continuam
  exclusivos da Recepcionista (`/recepcionista`).
